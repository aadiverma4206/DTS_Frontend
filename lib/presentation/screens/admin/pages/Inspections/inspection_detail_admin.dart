...
class InspectionDetailAdmin extends StatefulWidget {
  final int inspectionId;

  const InspectionDetailAdmin({super.key, required this.inspectionId});

  @override
  State<InspectionDetailAdmin> createState() => _InspectionDetailAdminState();
}

class _InspectionDetailAdminState extends State<InspectionDetailAdmin>
  ...
  Future<void> _loadDetail() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _inspection = data['inspection'] ?? data['data']?['inspection'];
        _items = List<Map<String, dynamic>>.from(
          data['items'] ?? data['data']?['items'] ?? [],
        );
        _checks = List<Map<String, dynamic>>.from(
          data['checks'] ?? data['data']?['checks'] ?? [],
        );
        _isLoading = false;
      });
  ...
  Widget _drugItemCard(Map<String, dynamic> item) {
    ...
    final drugName =
        item['drug_name']?.toString() ?? item['drug_id']?.toString() ?? '';
    final batchNo =
        item['batch_no']?.toString() ?? item['batch_id']?.toString() ?? '';
    final diff = physicalQty - systemQty;
    final hasMismatch = systemQty != physicalQty;
    ...
    return RepaintBoundary(
      child: Container(
        ...
        decoration: BoxDecoration(
          color: _C.surface,
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: hasMismatch ? _C.rose : _C.divider),
          boxShadow: [
            BoxShadow(
              color: _C.shadow,
              blurRadius: 10,
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    height: 42,
                    decoration: BoxDecoration(
                      color: hasMismatch ? _C.roseSoft : _C.indigoSoft,
                      borderRadius: BorderRadius.circular(12),
                    ),
                    child: Icon(
                      Icons.medication_rounded,
                      color: hasMismatch ? _C.rose : _C.indigo,
                      size: 20,
                    ),
                  ),
                  const SizedBox(width: 12),
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: hasMismatch ? _C.roseSoft : _C.greenSoft,
                      borderRadius: BorderRadius.circular(10),
                    ),
                    child: Row(
                      ...
                      children: [
                        Icon(
                          hasMismatch
                              ...
                              : Icons.check_circle_rounded,
                          size: 11,
                          color: hasMismatch ? _C.rose : _C.green,
                        ),
                        const SizedBox(width: 4),
                        Text(
                ...
                children: [
                  ...
                  Expanded(
                    child: _qtyChip(
                      'Physical',
                      physicalQty,
                      hasMismatch ? _C.rose : _C.green,
                      hasMismatch ? _C.roseSoft : _C.greenSoft,
                    ),
                  ),
                  if (hasMismatch) ...[
                    const SizedBox(width: 8),
  ...
  Widget _checkCard(Map<String, dynamic> check) {
    final value = check['check_value'] ?? check['value'] ?? 0;
    final isYes = value == 1 || value == 'yes' || value == 'pass';
