...
import '../../controller/sale_info_controller.dart';
...
class SaleListScreen extends StatelessWidget {
  ...
  final SaleInfoController controller = Get.put(SaleInfoController());
  ...
  Widget _userCard(Map<String, dynamic> item) {
    ...
    final dlNumber = (item['drug_license_no'] ?? '').toString();
    final initial = name.isNotEmpty ? name[0].toUpperCase() : '?';
    final itemType = (item['type'] ?? controller.selectedType.value).toString();
    final isRetailer = itemType == 'retailer';
    final chipColor = isRetailer ? _C.green : _C.blue;
    final chipBg = isRetailer ? _C.greenSoft : _C.blueSoft;
    return GestureDetector(
      onTap: () {
        final userId = controller.getUserId(item);
        Get.to(
          () => SaleDetailsScreen(
            userId: userId,
            type: itemType,
    ...
    return Obx(() {
      ...
      return RefreshIndicator(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (_, i) => _userCard(controller.filteredSales[i]),
        ),
      );
    });
  }
