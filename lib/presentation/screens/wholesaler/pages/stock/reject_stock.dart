...
import '../../controller/reject_stock_controller.dart';

class RejectStock extends StatefulWidget {
  const RejectStock({super.key});
  @override
  State<RejectStock> createState() => _RejectStockState();
}

class _RejectStockState extends State<RejectStock> {
  late final RejectStockController controller;
  ...
  static const Color _errorSoft = Color(0xFFFFECEF);
  final List<String> _filters = ["All", "Today", "This Week", "This Month"];
  @override
  void initState() {
    super.initState();
    controller = Get.put(RejectStockController());
  ...
  List<Map<String, dynamic>> get _filtered {
    return controller.stockList.where((item) {
      final matchSearch =
          ...
          (item["invoice_number"] ?? "").toString().toLowerCase().contains(
            _search,
          ) ||
          (item["retailer_name"] ?? "").toString().toLowerCase().contains(
            _search,
          );
  ...
  Widget _buildSummaryBanner(List<Map<String, dynamic>> items) {
    final totalQty = items.fold<int>(
      0,
      (sum, e) =>
          sum + (int.tryParse(e["rejected_qty"]?.toString() ?? "0") ?? 0),
    );
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _error.withOpacity(0.2), width: 1.2),
      ),
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.cancel_rounded, color: _error, size: 26),
          ),
          ...
          Column(
            ...
            children: [
              Text(
                "${items.length} Rejection${items.length != 1 ? 's' : ''}",
              ...
              Text(
                "Total rejected qty: $totalQty units",
                style: GoogleFonts.dmSans(
  ...
  Widget _buildCard(Map<String, dynamic> item) {
    final qty = item["rejected_qty"]?.toString() ?? "0";
    return Container(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Text(
                      _formatDate(item["invoice_date"]?.toString()),
                      style: GoogleFonts.dmSans(
                        color: _textSecondary,
                        fontSize: 12,
                      ),
          ...
          const Padding(
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    _BlinkingSkeleton(width: 15, height: 15, borderRadius: 3),
                    SizedBox(width: 8),
                    _BlinkingSkeleton(width: 150, height: 13, borderRadius: 3),
                  ],
                ),
                SizedBox(height: 8),
                Row(
                  children: [
                    _BlinkingSkeleton(width: 15, height: 15, borderRadius: 3),
                    SizedBox(width: 8),
                    _BlinkingSkeleton(width: 180, height: 13, borderRadius: 3),
                  ],
                ),
                SizedBox(height: 8),
                Row(
                  children: [
                    _BlinkingSkeleton(width: 15, height: 15, borderRadius: 3),
                    SizedBox(width: 8),
                    _BlinkingSkeleton(width: 100, height: 13, borderRadius: 3),
                  ],
                ),
                SizedBox(height: 10),
                Row(
      ...
      body: GestureDetector(
        ...
        child: Obx(() {
          ...
          return RefreshIndicator(
            ...
            onRefresh: controller.loadRejectedStocks,
                        ...
                        Container(
                          ...
                          child: TextField(
                            ...
                            decoration: InputDecoration(
                              hintText:
                                  "Search drug, batch, invoice, retailer...",
